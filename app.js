...
function weekLabel(key) { const [year, weekPart] = key.split("-W"); return `Week ${Number(weekPart)}, ${year}`; }
...
function renderWeekOptions() {
  ...
  const weeks = [...new Set(state.entries.map((entry) => isoWeekKey(entry.date)))].sort().reverse();
  select.innerHTML = "";
...
function renderWeeklyLeaderboard() {
  ...
  const entries = week ? state.entries.filter((entry) => isoWeekKey(entry.date) === week) : [];
